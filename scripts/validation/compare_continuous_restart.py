#!/usr/bin/env python3
"""
Quantitatively compare a continuous PFLOTRAN run with a checkpoint/restart continuation.

The script compares both geomechanics and, optionally, flow HDF5 outputs at
common output times. It is intended for restart-equivalence validation. It validates that the two geomechanics files use the same
...
regions, and writes:
  ...
  * publication-quality continuous/restart strain overlays;
  * restart-minus-continuous strain-difference plots;
...
Example
-------
python3 compare_continuous_restart.py \
  --continuous-geomech north_avant_v5_continuous_injection_smoke-geomech.h5 \
  --restart-geomech north_avant_v5_restart_injection_smoke-geomech.h5 \
  --continuous-flow north_avant_v5_continuous_injection_smoke.h5 \
  --restart-flow north_avant_v5_restart_injection_smoke.h5 \
  ...
  --region HEC=hec.vset \
  --output-dir compare_continuous_restart_v5
"""
...
def numeric_metrics(one: np.ndarray, two: np.ndarray) -> dict[str, float]:
    ...
    return {
        "continuous_mean": float(np.mean(a)),
        "restart_mean": float(np.mean(b)),
        "mean_delta_restart_minus_continuous": float(np.mean(delta)),
        ...
        "continuous_l2_norm": norm_a,
        "restart_l2_norm": norm_b,
        "delta_l2_norm": norm_delta,
        "symmetric_relative_l2": norm_delta / scale,
        "continuous_min": float(np.min(a)),
        "continuous_max": float(np.max(a)),
        "restart_min": float(np.min(b)),
        "restart_max": float(np.max(b)),
    }
...
def region_metrics(
    ...
    return {
        ...
        "continuous_mean": one_mean,
        "restart_mean": two_mean,
        "mean_delta_restart_minus_continuous": two_mean - one_mean,
        "symmetric_percent_difference_of_means": (
            100.0 * (two_mean - one_mean) / denominator
        ),
        "continuous_spatial_std": float(np.std(a)),
        "restart_spatial_std": float(np.std(b)),
        ...
        for one_group_info, two_group_info in pairs:
            ...
            for key in common_keys:
                ...
                global_rows.append(
                    {
                        ...
                        "field_key": key,
                        "continuous_field_name": one_arrays[key]["original_name"],
                        "restart_field_name": two_arrays[key]["original_name"],
                        **metrics,
                    }
                )
                ...
                for region_name, indices in region_indices.items():
                    ...
                    series_entry["continuous"].append(
                        float(rmetrics["continuous_mean"])
                    )
                    series_entry["restart"].append(
                        float(rmetrics["restart_mean"])
                    )
                    series_entry["delta"].append(
                        float(rmetrics["mean_delta_restart_minus_continuous"])
                    )
...
def plot_region_strain_comparisons(
    ...
    for region, fields in series.items():
        ...
        for component, key in component_keys:
            ...
            one = multiplier * np.asarray(values["continuous"], dtype=float)
            two = multiplier * np.asarray(values["restart"], dtype=float)
        ...
        ax.set_title(
            f"{region}: continuous versus restart mean strain components"
        )
        ...
        ax.legend(
            handles=[
                Line2D(
                    ...
                    color="0.15", label="Restart",
                ),
                Line2D(
                    [0], [0], linestyle="--", marker="x",
                    color="0.15", label="Continuous",
                ),
            ],
            title="Run",
            loc="upper left",
        ...
        stem = output_dir / f"{safe_name(region)}_strain_continuous_restart"
        ...
        ax.set_xlabel("Time [h]")
        ax.set_ylabel(
            "Restart minus continuous " + ylabel.lower()
        )
        ax.set_title(
            f"{region}: restart-minus-continuous strain difference"
        )
        ax.ticklabel_format(axis="y", style="sci", scilimits=(-3, 3))
...
def compute_magnitude_series(
    ...
    one = np.sqrt(
        sum(np.asarray(item["continuous"], dtype=float) ** 2 for item in selected)
    )
    two = np.sqrt(
        sum(np.asarray(item["restart"], dtype=float) ** 2 for item in selected)
    )
...
def plot_displacement_comparison(
    ...
    ax.set_title("Continuous versus restart displacement response")
    ax.ticklabel_format(axis="y", style="sci", scilimits=(-3, 3))
    ...
    ax.legend(
        handles=[
            Line2D([0], [0], color="0.15", marker="o", label="Restart"),
            Line2D(
    ...
    stem = output_dir / "displacement_magnitude_continuous_restart"
    save_figure(fig, stem, dpi)
    outputs.extend([str(stem.with_suffix(".png")), str(stem.with_suffix(".pdf"))])
    return outputs
...
def plot_pressure_comparison(
    ...
    for region, fields in series.items():
        ...
        time = np.asarray(values["time_hours"], dtype=float)
        one = np.asarray(values["continuous"], dtype=float)
        two = np.asarray(values["restart"], dtype=float)
    ...
    ax.set_title("Continuous versus restart flow-pressure response")
    ax.ticklabel_format(axis="y", style="sci", scilimits=(-3, 3))
    ...
    stem = output_dir / "liquid_pressure_continuous_restart"
    save_figure(fig, stem, dpi)
    return [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Compare matched PFLOTRAN continuous and restart outputs."
    )
    parser.add_argument("--continuous-geomech", type=Path, required=True)
    parser.add_argument("--restart-geomech", type=Path, required=True)
    parser.add_argument("--continuous-flow", type=Path)
    parser.add_argument("--restart-flow", type=Path)
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("compare_continuous_restart"),
    )
    parser.add_argument(
    ...
    one_geomech = args.continuous_geomech.expanduser().resolve()
    two_geomech = args.restart_geomech.expanduser().resolve()
    ...
    print("Continuous/restart comparison")
    print("==========================")
    print(f"Mechanics vertices: {mechanics_count:,}")
    print(
        "Maximum continuous/restart vertex difference: "
    ...
    flow_files_supplied = args.continuous_flow is not None or args.restart_flow is not None
    if flow_files_supplied:
        if args.continuous_flow is None or args.restart_flow is None:
            raise RuntimeError(
                "Supply both --continuous-flow and --restart-flow, or neither"
            )
        ...
        one_flow = args.continuous_flow.expanduser().resolve()
        two_flow = args.restart_flow.expanduser().resolve()
    ...
    summary = {
        "continuous_geomechanics": str(one_geomech),
        "restart_geomechanics": str(two_geomech),
